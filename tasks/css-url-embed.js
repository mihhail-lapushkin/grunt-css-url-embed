...
module.exports = function(grunt) {
  ...
  function embedUrls(f, options) {
    try {
      ...
      var targetUrls = allUrls.filter(function(url) {
        return !url.match('(data:|http[s]*:)');
      });
      var uniqTargetUrls = grunt.util._.uniq(targetUrls);
      var extractedUrls = uniqTargetUrls.map(function(url) {
      ...
      extractedUrls.forEach(function(rawUrl, i) {
        ...
        ext = (ext.indexOf('#') >= 0) ? ext.match(/(?:(.*?)(?:#))/g) : ext;
        ext = ext.toString();
        var excluded = false;
        options.exclude.forEach(function(exclusion) {
          if(!excluded) {
            exclusion = exclusion.toString();
            //grunt.log.writeln(exclusion + ' : ' + ext);
            if (exclusion === ext) {
              grunt.log.writeln('Exclusion "' + exclusion + '" matched "' + rawUrl + '"');
              excluded = true;
            }
          }
        });
